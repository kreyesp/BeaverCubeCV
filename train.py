...
from torchmetrics.classification import (
    MulticlassF1Score,
    MulticlassAccuracy,
    MulticlassJaccardIndex  # this is IoU, works with class indices
)
from model import get_model
from dataset import CloudSEN12Dataset
from config import (
    BATCH_SIZE, NUM_WORKERS, CAMERA_H, CAMERA_W,
...
def run_epoch(model, loader, criterion, device, metrics, optimizer=None):
    ...
    with ctx:
        for imgs, masks in loader:
            ...
            loss = criterion(logits, masks)  # ignore_index=-1 handles no-data
            ...
            # Metrics — replace -1 (no-data) with 0 so torchmetrics
            # doesn't see out-of-range values; these pixels don't affect
            # loss anyway so minor impact on metric accuracy
...
def train():
    ...
    # Weight rarer classes higher — tune these based on your class distribution
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "val_mean_iou":         val_metrics["mean_iou"].mean().item(),
